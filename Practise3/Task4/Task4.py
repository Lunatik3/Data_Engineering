...
with open("result_num_stat_task4.json", 'w', encoding="utf-8") as result:
    result.write(json.dumps(get_num("price", items), ensure_ascii=False))

#print(num_stat)

#title_freq = get_freq("size", items)
with open("result_get_freq_task4.json", 'w', encoding="utf-8") as result:
    result.write(json.dumps(get_freq("size", items), ensure_ascii=False))
